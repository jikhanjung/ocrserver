...
> **⚠️ 정정 (같은 날 늦게, devlog 039 조사 중 발견)**
>
> 이 문단은 원래 "크래시 시점 vllm 로그가 logrotate 로 유실됐다" 고
> 적혀 있었으나 **오진이었음**. 로그는 하나도 유실되지 않았음 —
> 컨테이너 `json.log` 는 단일 파일 37.6MB 에 06-24~07-27 전 구간
> 195,954 줄이 온전히 존재하고, docker 로테이션 설정도 logrotate 항목도
> 애초에 없음(=로테이션 자체가 일어난 적 없음).
>
> 실제 원인은 **`docker logs` CLI 가 이 파일에서 부분만 반환**한 것.
> 같은 컨테이너에 대해 `docker logs` 전체는 06-24~06-25 만,
> `--tail 100000` 은 07-11~07-23 만, `--tail 15` 는 07-27 을 반환하고,
> `--since/--until` 로 07-26 구간을 요청하면 0.08 초 만에 0 줄을 반환했음
> (타임아웃 아님). 원인 미상이나 **`docker logs` 를 유실 판정 근거로
> 쓰면 안 된다**는 교훈. 원본을 직접 읽을 것:
>
> ```bash
> docker run --rm -v /var/lib/docker/containers:/c:ro nginx:alpine \
>   cat /c/<containerId>/<containerId>-json.log
> ```
>
> 이 로그를 실제로 읽어서 크래시 원인을 규명한 결과가 **devlog 039**.
> 결론만: EngineCore 는 **SIGSEGV** 로 죽고 있었고, 충돌 지점이 10 건
> 모두 서로 무관해서 메모리 손상 시그니처에 해당함.
...
- ~~**docker 로그 로테이션 미설정으로 크래시 구간 로그 유실**~~ →
  **오진. 취소** (위 정정 박스 참조). 로그는 온전히 남아 있었고
  `docker logs` CLI 가 부분만 반환한 것이었음. 다만 `daemon.json` 에
  `max-size`/`max-file` 이 없어 **무한 증가**하는 것은 사실이므로
  (llm 컨테이너 37.6MB / 33일), 디스크 관리 차원의 설정은 여전히 유효.
  단 이건 유실 방지가 아니라 증가 억제 목적이고, 오히려 켜면 **오래된
  크래시 로그가 실제로 지워지므로** 조사 중에는 켜지 말 것.
