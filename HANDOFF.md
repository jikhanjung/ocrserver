...
chandra-b   honestjung/ocrserver:latest   Up 26h (이전 세션 시작분, profile=ocr)
nginx       nginx:alpine                  Up 26h
wrapper     honestjung/ocrwrapper:0.1.2   Up 3h   ← 운영 중 (0.1.3, 0.1.4 빌드/push 했지만 미배포)
```
...
- done: 2398, done_with_errors: 6, failed: 5, processing: 1
- **현재 OCR 작업 진행 중** — `processing` 1건. 끝나기 전엔 wrapper 교체 보류.

### compose 파일 상태
- `/home/jikhanjung/projects/ocrserver/docker-compose.yml` : wrapper `0.1.4`
- `/srv/ocrserver/docker-compose.yml` : wrapper `0.1.3` ← **dev tree 와 1 minor 차이**.
  swap 직전에 `cp /home/jikhanjung/projects/ocrserver/docker-compose.yml /srv/ocrserver/`
  먼저 한 뒤 `up -d wrapper` 해야 함.

## 곧 해야 할 작업

1. **현재 OCR 작업 끝나면 wrapper 0.1.4 으로 swap**
   ```bash
   cp /home/jikhanjung/projects/ocrserver/docker-compose.yml /srv/ocrserver/docker-compose.yml
   cd /srv/ocrserver && docker compose up -d wrapper
   ```
   compose 가 desired tag `:0.1.4` 와 떠있는 `:0.1.2` mismatch 감지 → recreate.
   `--force-recreate` 불필요. lifespan resume 이 in-flight job 픽업하므로
   데이터 손실 없음 ([[devlog/20260515_010_lifespan_resume]]).

   교체 후 변경 사항 한꺼번에 반영:
   - 페이지 단위 처리량 (0.1.1)
   - 새 `/status` — 2 GPU + LLM 부재 모드 대응 (0.1.2)
   - `/metrics` 차트 활성 구간 평균 텍스트 (0.1.3)
   - 헤더 통일 + 대시보드 페이지네이션 + "통계" 라벨 (0.1.4)
